...
public class TestRequestModelHandler: IHorseRequestHandler<TestRequestModel, TestResponseModel>
{
    ...
    public Task<ErrorResponse> OnError(Exception exception, TestRequestModel request, HorseMessage rawMessage,
        HorseClient client, CancellationToken cancellationToken = default)
    {
        return exception switch
        {
            SampleException => Task.FromResult(new ErrorResponse { Reason = "Reason is no matter.", ResultCode = HorseResultCode.Failed }),
