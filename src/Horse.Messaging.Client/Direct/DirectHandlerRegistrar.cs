...
public class DirectHandlerRegistrar
{
    ...
    private DirectHandlerRegistration CreateHandlerRegistration(ModelTypeInfo typeInfo, Func<IHandlerFactory> consumerFactoryBuilder)
    {
        ...
        switch (typeInfo.Source)
        {
            ...
            {
                Type executorType = typeof(DirectHandlerExecutor<>);
                Type executorGenericType = executorType.MakeGenericType(typeInfo.ModelType);
                executor = (ExecutorBase) Activator.CreateInstance(executorGenericType,
            ...
            {
                Type executorType = typeof(RequestHandlerExecutor<,>);
                Type executorGenericType = executorType.MakeGenericType(typeInfo.ModelType, typeInfo.ResponseType);
                executor = (ExecutorBase) Activator.CreateInstance(executorGenericType,
                    typeInfo.ConsumerType,
                    consumerInstance,
                    consumerFactoryBuilder);
