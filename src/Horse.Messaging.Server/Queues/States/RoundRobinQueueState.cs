	...
	internal class RoundRobinQueueState: IQueueState
	{
		...
		public async Task<PushResult> Push(QueueMessage message)
		{
			try
			{
				...
				Tuple<QueueClient, int> tuple = await GetNextAvailableRRClient(_roundRobinIndex, message, _queue.Options.Acknowledge == QueueAckDecision.WaitForAcknowledge);
				QueueClient cc = tuple.Item1;
				if (cc != null) _roundRobinIndex = tuple.Item2;
		...
		/// </summary>
		internal async Task<Tuple<QueueClient, int>> GetNextAvailableRRClient(int currentIndex, QueueMessage message, bool waitForAcknowledge)
		{
			List<QueueClient> clients = _queue.ClientsClone;
			if (clients.Count == 0)
			...
			while (true)
			{
				...
				for (int i = 0; i < clients.Count; i++)
				{
					...
					if (waitForAcknowledge && client.ProcessDeadline < DateTime.UtcNow)
					{
