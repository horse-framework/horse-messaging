...
namespace Horse.Mq.Bus
{
	...
	public static class HorseConnectorExtensions
	{
		...
		public static IServiceCollection AddHorseBus(this IServiceCollection services, Action<HorseConnectorBuilder> config)
		{
			...
			AddInterceptorsMicrosoftDI(services, connector, builder);
		...
		{
			foreach (Tuple<ServiceLifetime, Type> pair in builder.AssemblyConsumers)
			{
				...
				foreach (Type type in types)
					AddTypeIntoCollection(services, pair.Item1, type);	
			}
			...
			foreach (Tuple<ServiceLifetime, Type> pair in builder.IndividualConsumers)
			{
				...
				AddTypeIntoCollection(services, pair.Item1, pair.Item2);
			}
		}
		
		private static void AddInterceptorsMicrosoftDI(IServiceCollection services, HmqStickyConnector connector, HorseConnectorBuilder builder)
		{
			foreach (Tuple<ServiceLifetime, Type> pair in builder.AssemblyInterceptors)
			{
				IEnumerable<Type> interceptorTypes = connector.Observer
											   .RegisterAssemblyInterceptors(pair.Item2);

				foreach (Type type in interceptorTypes)
					AddTypeIntoCollection(services, pair.Item1, type);
			}

			foreach (Tuple<ServiceLifetime, Type> pair in builder.IndividualInterceptors)
				AddTypeIntoCollection(services, pair.Item1, pair.Item2);
		}

		private static void AddTypeIntoCollection(IServiceCollection services, ServiceLifetime lifetime, Type consumerType)
		{
			switch (lifetime)
			{
